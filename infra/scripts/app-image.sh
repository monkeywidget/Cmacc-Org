...
case ${1:-all} in
  app|all) b -f app/Dockerfile --target app -t "cmacc-app:dev-$REV" && echo "built cmacc-app:dev-$REV" ;;&
  test|all) b -f app/Dockerfile --target test -t cmacc-app:test && echo "built cmacc-app:test" ;;&
  templates|all) b -f infra/templates.Dockerfile -t "cmacc-templates:dev-$REV" && echo "built cmacc-templates:dev-$REV" ;;
esac
